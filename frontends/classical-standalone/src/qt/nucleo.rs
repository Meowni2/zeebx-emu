...
pub struct Volta {
    ...
    pub fechou: bool,
    /// O que mostrar na linha de estado, quando mudou: o título do jogo novo ou o motivo da parada.
...
impl Nucleo {
    ...
    /// A tela do jogo aberto, como está agora.
    pub fn tela(&self) -> Option<&Framebuffer> {
        self.partida.as_ref().map(|partida| partida.sessao().screen())
    }

    /// O título do jogo aberto e onde ele desenha.
    pub fn estado(&self) -> String {
        let Some(partida) = &self.partida else {
            return String::new();
        };
        ...
        format!("{} — {onde}", partida.sessao().title())
    }
    ...
    pub fn passo(&mut self) -> Volta {
        ...
        no_contexto(self.gl.is_some(), || partida.avanca(&pads, movimentos, teclado, limite));
        ...
        if volta.estado.is_none() {
            volta.estado = self
                .partida
                .as_ref()
                .and_then(|partida| partida.sessao().stopped_reason());
        }
        volta
    }
}
