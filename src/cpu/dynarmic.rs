...
impl DynarmicCpu {
    /// **Ganchos de depuração que o Dynarmic ainda não oferece.**
    ///
    /// `trace_code`, `watch`, `set_wall_limit`, `enable_profile`, `steps`, `writes`, `profile` e
    /// `wall_expired` nasceram como ferramentas do backend antigo, que parava a execução onde se
    /// pedia. O Dynarmic recompila blocos, e ainda não oferece esses ganchos com a mesma
    /// granularidade. A recusa explícita é melhor que um `--trace` que não mostra nada.
    ///
    ...
    fn sem_gancho_de_debug(&self) -> CpuError {
        CpuError("este gancho de depuração ainda não existe no backend Dynarmic".to_string())
    }

    pub fn trace_code(&mut self, _begin: u32, _end: u32, _limite: usize) -> Result<(), CpuError> {
        Err(self.sem_gancho_de_debug())
    }
